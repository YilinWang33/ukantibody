...
def run_proteinmpnn(
    ...
    # Base command without optional parameters (Fixed omit_AAs issue and bash redirection bug)
    base_command = f"""
    ...
    if omit_AAs is not None:
        base_command += f" --omit_AAs {omit_AAs}"
    if ca_only:
    ...
    # Appending redirection at the very end to avoid messing up parameters
    if not verbose:
...
def scRMSD(
    ...
    # Extract real chain IDs from PDB directly to handle multiple chains correctly
    chain_ids_in_file = []
    ...
    if not use_pdb_seq:
        ...
        gen_seqs = run_proteinmpnn(
            pdb_file_path,
            tmp_path,
            all_chains=real_unique_chains,       # Use dynamically extracted real chains
            pdb_path_chains=real_unique_chains,  # Use dynamically extracted real chains
            num_seq_per_target=num_seq_per_target,
        ...
        gen_seqs = [extract_seq_from_pdb(pdb_file_path)]  
        suffix = "pdb"
...
def sc_sequence_recovery(
    ...
    # Same fix here: extract real chain IDs
    chain_ids_in_file = []
    with open(pdb_file_path, 'r') as f:
        for line in f:
            if line.startswith("ATOM") or line.startswith("HETATM"):
    ...
    mpnn_seqs = run_proteinmpnn( 
        pdb_file_path,
        tmp_path,
        all_chains=real_unique_chains,
        pdb_path_chains=real_unique_chains,
        num_seq_per_target=num_seq_per_target,
        sampling_temp=pmpnn_sampling_temp,
        fix_pos=motif_index,
    )  
