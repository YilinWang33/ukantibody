...
def generate_motif_indices(
    ...
    for combo in combos:
        ...
        all_res_indices.append(res_idx_list)
...
def parse_motif(
    ...
    # 修改这里的解包
    lengths, motif_indices, out_strs, all_res_indices, all_chain_indices = generate_motif_indices(
        contig_string, motif_min_length, motif_max_length, nsamples
    )
    
    motif_masks, x_motifs, residue_types = [], [], []
    res_indices_out, chain_indices_out = [], [] # NEW
    # print(lengths)
    # print(motif_indices)
    # print(out_strs)
    for length, motif_index, out_str, res_idx, chain_idx in zip(lengths, motif_indices, out_strs, all_res_indices, all_chain_indices):
        ...
        res_indices_out.append(torch.tensor(res_idx, dtype=torch.int64)) # NEW
        chain_indices_out.append(torch.tensor(chain_idx, dtype=torch.int64)) # NEW

    return lengths, motif_masks, x_motifs, residue_types, out_strs, res_indices_out, chain_indices_out
