...
class AntibodyMotifMaskTransform:
    """
    专门为抗体设计：
    motif_mask = 1 (Fixed) -> Framework + Antigen
    motif_mask = 0 (Diffused/Generated) -> CDRs
    """
    def __call__(self, graph: Data) -> Data:
        # 1. 基础检查
        if not hasattr(graph, 'is_cdr'):
            # 如果没有 CDR 信息（比如纯抗原），全设为 1（固定）或随机
            # 这里默认全固定，防止报错
            graph.motif_mask = torch.ones_like(graph.coord_mask, dtype=torch.bool)
        else:
            # 2. 生成 motif_mask
            # is_cdr 为 True 的地方是要生成的 (Unknown)，所以 motif_mask 设为 0
            # is_cdr 为 False 的地方是已知的 (Fixed Motif)，motif_mask 设为 1
            # 注意维度：motif_mask 需要是 [N, 37] 或者 [N] 广播
            
            is_cdr = graph.is_cdr  # [N]
            motif_mask = ~is_cdr   # [N], True=Fixed, False=Gen
            
            # 扩展到原子维度 [N, 37]
            graph.motif_mask = motif_mask.unsqueeze(-1).expand_as(graph.coord_mask)

        # 3. 构造 x_motif (提示给模型的已知结构)
        # 将需要生成的部分坐标抹零
        if hasattr(graph, 'coords_nm'): # 确保已经转为纳米单位
            graph.x_motif = graph.coords_nm * graph.motif_mask.unsqueeze(-1)
        else:
            # 如果还没转 nm，假设 coords 存在
            graph.x_motif = graph.coords * graph.motif_mask.unsqueeze(-1)

        # 4. 构造 seq_motif (序列提示)
        # 通常抗体设计我们知道全序列，所以这里 seq_motif 可以是全序列
        # 或者如果你想做序列+结构共设计，也可以把 CDR 的 seq mask 掉
        # 这里假设 Sequence 是已知的 Condition
        graph.seq_motif_mask = torch.ones_like(graph.residue_type, dtype=torch.bool)
        graph.seq_motif = graph.residue_type
