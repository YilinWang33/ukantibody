...
class GenDataset(Dataset):
    ...
    def __init__(
        ...
        if nres is not None:
            ...
            # 【修改这里】：原来只有3个，现在要用5个变量来接收 generate_motif_info 的返回
            self.motif_masks, self.x_motifs, self.residue_types, self.res_indices, self.chain_indices = (
    ...
    def flatten_motif(self, max_nsamples: int):
        """Flatten the list to make sure each data point have no more than max_nsamples"""
        nres, cath_codes, nsamples = [], [], []
        masks, motif_masks, x_motifs, residue_types = [], [], [], []
        res_indices_out, chain_indices_out = [], [] # NEW
        for i in range(len(self.nsamples)):
            for j in range(0, self.nsamples[i], max_nsamples):
                ...
                if j + max_nsamples <= self.nsamples[i]:
                    ...
                    res_idx = self.res_indices[j : j + max_nsamples]       # NEW
                    chain_idx = self.chain_indices[j : j + max_nsamples]   # NEW
                else:
                    nsamples.append(self.nsamples[i] - j)
                    ...
                    res_idx = self.res_indices[j : self.nsamples[i]]       # NEW
                    chain_idx = self.chain_indices[j : self.nsamples[i]]   # NEW
                ...
                padded_res_idx = torch.nn.utils.rnn.pad_sequence(res_idx, batch_first=True, padding_value=0)     # NEW
                padded_chain_idx = torch.nn.utils.rnn.pad_sequence(chain_idx, batch_first=True, padding_value=0) # NEW

                masks.append(padded_mask)
                motif_masks.append(padded_motif_mask)
                x_motifs.append(padded_x_motif)
                residue_types.append(padded_residue_type)
                res_indices_out.append(padded_res_idx)     # NEW
                chain_indices_out.append(padded_chain_idx) # NEW
                nres.append(padded_mask.shape[1])
        return nres, cath_codes, nsamples, masks, motif_masks, x_motifs, residue_types, res_indices_out, chain_indices_out
